...
class HomeViewModel : ViewModel() {
    ...
    // Lista base com cores e empresas — mesma do seu HomeFragment atual
            ...
            repository.getStatusLinhas()
                .onSuccess { statusMap ->
                    _linhas.value = linhasBase.map { linha ->
                        ...
                        linha.copy(
                            status       = triple?.first  ?: "Sem informacao",
                ...
                .onFailure {
                    ...
                    // Mantém a lista base com status padrão
                }
    ...
    private fun iniciarAutoRefresh() {
        viewModelScope.launch {
            while (true) {
                delay(6 * 60 * 1000L) // 6 minutos = 10 req/hora, seguro dentro do limite
                fetchStatus()
            }
        }
    }
